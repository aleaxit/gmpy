...
.. warning::
   ...
   The gmpy2 has support for python runtime with the GIL disabled.  To make
   properly, this requires using thread-local storage for MPFR data, such as as
   flags, the exponent range, the default precision, etc.  That is, MPFR should
   be build with the ``--enable-thread-safe`` configure option.
