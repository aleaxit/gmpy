...
GMPY_mpz_is_fermat_prp(PyObject *self, PyObject *args)
{
    ...
    n = GMPy_MPZ_From_Integer(PyTuple_GET_ITEM(args, 0));
    a = GMPy_MPZ_From_Integer(PyTuple_GET_ITEM(args, 1));
    if (!a || !n) {
        TYPE_ERROR("is_fermat_prp() requires 2 integer arguments");
        goto cleanup;
    }
...
GMPY_mpz_is_fibonacci_prp(PyObject *self, PyObject *args)
{
    ...
    n = GMPy_MPZ_From_Integer(PyTuple_GET_ITEM(args, 0));
    p = GMPy_MPZ_From_Integer(PyTuple_GET_ITEM(args, 1));
    q = GMPy_MPZ_From_Integer(PyTuple_GET_ITEM(args, 2));
    if (!n || !p || !q) {
        TYPE_ERROR("is_fibonacci_prp() requires 3 integer arguments");
        goto cleanup;
    }
