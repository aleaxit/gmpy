    ...
    if (CHECK_MPZANY(x)) {
        if (PyIntOrLong_Check(y)) {
            ...
            if (overflow) {
                mpz_inoc(tempz);
                mpz_set_PyIntOrLong(tempz, y);
                mpz_sub(result->z, Pympz_AS_MPZ(x), tempz);
                mpz_cloc(tempz);
            }
            else if (temp_si >= 0) {
                mpz_sub_ui(result->z, Pympz_AS_MPZ(x), temp_si);
        ...
        if (CHECK_MPZANY(y)) {
            mpz_sub(result->z, Pympz_AS_MPZ(x), Pympz_AS_MPZ(y));
            return (PyObject*)result;
        }
    }
    ...
    if (CHECK_MPZANY(y)) {
        if (PyIntOrLong_Check(x)) {
            ...
            if (overflow) {
                mpz_inoc(tempz);
                mpz_set_PyIntOrLong(tempz, x);
                mpz_sub(result->z, tempz, Pympz_AS_MPZ(y));
                mpz_cloc(tempz);
            }
            else if (temp_si >= 0) {
                mpz_ui_sub(result->z, temp_si, Pympz_AS_MPZ(y));
            }
            else {
                mpz_add_ui(result->z, Pympz_AS_MPZ(y), -temp_si);
...
/* Implement __sub__ for Pympz. On entry, one of the two arguments must
 * be a Pympz. If the other object is an Integer, add and return a Pympz.
 * If the other object isn't a Pympz, call the appropriate function. If
 * no appropriate function can be found, return NotImplemented. */
    ...
    if (CHECK_MPZANY(x)) {
        if (PyIntOrLong_Check(y)) {
            ...
            if (overflow) {
                mpz_inoc(tempz);
                mpz_set_PyIntOrLong(tempz, y);
                mpz_mul(result->z, Pympz_AS_MPZ(x), tempz);
                mpz_cloc(tempz);
            }
            else {
                mpz_mul_si(result->z, Pympz_AS_MPZ(x), temp_si);
            }
            return (PyObject*)result;
        }

        if (CHECK_MPZANY(y)) {
            mpz_mul(result->z, Pympz_AS_MPZ(x), Pympz_AS_MPZ(y));
            return (PyObject*)result;
        }
    }
    ...
    if (CHECK_MPZANY(y)) {
        if (PyIntOrLong_Check(x)) {
            ...
            if (overflow) {
                mpz_inoc(tempz);
                mpz_set_PyIntOrLong(tempz, x);
                mpz_mul(result->z, Pympz_AS_MPZ(y), tempz);
                mpz_cloc(tempz);
            }
            else {
                mpz_mul_si(result->z, Pympz_AS_MPZ(y), temp_si);
            }
            return (PyObject*)result;
        }
    }
...
Pympz_FloorDiv_Integer(PyObject *x, PyObject *y, GMPyContextObject *context)
{
    ...
    if (CHECK_MPZANY(x)) {
        if (PyIntOrLong_Check(y)) {
            ...
            if (overflow) {
                mpz_inoc(tempz);
                mpz_set_PyIntOrLong(tempz, y);
                mpz_fdiv_q(result->z, Pympz_AS_MPZ(x), tempz);
                mpz_cloc(tempz);
            }
            else if (temp_si > 0) {
    ...
    if (CHECK_MPZANY(y)) {
        ...
        if (PyIntOrLong_Check(x)) {
            mpz_inoc(tempz);
            mpz_set_PyIntOrLong(tempz, x);
            mpz_fdiv_q(result->z, tempz, Pympz_AS_MPZ(y));
            mpz_cloc(tempz);
            return (PyObject*)result;
        }
    }
...
{
    PympzObject *tempx, *tempy, *result;
    mpz_t tempz;
    mpir_si temp_si;
    int overflow;
    ...
    if (CHECK_MPZANY(x)) {
        if (PyIntOrLong_Check(y)) {
            ...
            if (overflow) {
                mpz_inoc(tempz);
                mpz_set_PyIntOrLong(tempz, y);
                mpz_fdiv_r(result->z, Pympz_AS_MPZ(x), tempz);
                mpz_cloc(tempz);
            }
            else if (temp_si > 0) {
                mpz_fdiv_r_ui(result->z, Pympz_AS_MPZ(x), temp_si);
            }
            else if (temp_si == 0) {
                ZERO_ERROR("division or modulo by zero");
                Py_DECREF((PyObject*)result);
                ...
                mpz_cdiv_r_ui(result->z, Pympz_AS_MPZ(x), -temp_si);
            }
            return (PyObject*)result;
        }
        if (CHECK_MPZANY(y)) {
            ...
            mpz_fdiv_r(result->z, Pympz_AS_MPZ(x), Pympz_AS_MPZ(y));
            return (PyObject*)result;
        }
    }
    ...
    if (CHECK_MPZANY(y)) {
        ...
        if (PyIntOrLong_Check(x)) {
            mpz_inoc(tempz);
            mpz_set_PyIntOrLong(tempz, x);
            mpz_fdiv_r(result->z, tempz, Pympz_AS_MPZ(y));
            mpz_cloc(tempz);
            return (PyObject*)result;
        }
    }
